...
const compareAnswers = (rightAnswers, studentAnswer) => {
    if (rightAnswers === studentAnswer) {
        return 1;
...
const countPoints = (rightAnswers, studentAnswer, action) => {
    let contador = 0;
    for (let index = 0; index < rightAnswers.length; index += 1) {
        const actionReturn = action(rightAnswers[index], studentAnswer[index]);
